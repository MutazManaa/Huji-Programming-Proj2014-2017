...
train_acc = zeros(size(momentum,2),size(learningRates,2),5);
test_acc = zeros(size(momentum,2),size(learningRates,2),5);
...
for i = 1:size(momentum,2)
    for j = 1:size(learningRates,2)
        for l = 1:5
            ...
            mu     = momentum(i);   % Momentum variable
...
[idealAvgTrainErr, bestLRTrain] = max(avgTrainErr,[],2);
[idealAvgTestErr, bestLRTest] = max(avgTestErr,[],2);
idealAvgTrainErr = 1 - idealAvgTrainErr;
idealAvgTestErr = 1 - idealAvgTestErr;
figure;
plot(momentum,[idealAvgTrainErr idealAvgTestErr]);
title('Avg Train/Test Error VS Momentum');
legend('Avg Train Error','Avg Test Error');
fprintf('AVG Train error: %d ---',idealAvgTrainErr);
fprintf('\n');
fprintf('AVG Test error: %d ---',idealAvgTestErr);
fprintf('\n');
fprintf('best learning rate train: %d ---',bestLRTrain);
fprintf('\n');
fprintf('best learning rate test: %d ---', bestLRTest);
end