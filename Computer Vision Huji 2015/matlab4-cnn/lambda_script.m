...
train_acc = zeros(size(lambdas,2),5);
test_acc = zeros(size(lambdas,2),5);
...
for i = 1:size(lambdas,2)
    ...
    for l = 1:5
        ...
        train_acc(i,l) = good_pred/(good_pred+bad_pred);
        ...
        test_acc(i,l) = good_pred/(good_pred+bad_pred);
        fprintf('Test accuracy: %g\n',test_acc(i,l));
    end
end

lambdas = [0 1e-4 5e-4 1e-3 5e-3 1e-2 5e-2];
TrainErr = sum(train_acc,2);
TestErr = sum(test_acc,2);
avgTrainErr = TrainErr / 5;
avgTestErr = TestErr / 5;
avgTrainErr = 1 -  avgTrainErr;
avgTestErr = 1 - avgTestErr;
figure;
plot(lambdas,[avgTrainErr avgTestErr]);
title('Avg Train/Test Error VS Lambda');
