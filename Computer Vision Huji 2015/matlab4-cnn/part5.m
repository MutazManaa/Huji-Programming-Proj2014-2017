...
for l = 1:5
    ...
    net = ConvNet(net_arch,GPU,'Xavier');  % Zeros initialization of conv and affine layers
    ...
    eta    = @itNumbers; % Learning rate - eta(t) returns the value for iteration t
    ...
    train_acc(l) = good_pred/(good_pred+bad_pred);
    ...
    test_acc(l) = good_pred/(good_pred+bad_pred);
    fprintf('Test accuracy: %g\n',test_acc);
end

TrainErr = 1 - train_acc;
TestErr = 1 - test_acc;

avgTrainErr = mean(TrainErr);
avgTestErr = mean(TestErr);
fprintf('Average Train Error %.3f',avgTrainErr);
fprintf('\n');
fprintf('Average Test Error %.3f',avgTestErr);
end
function [eta]= itNumbers(it)
if it < 7500
    eta = 0.05;
else
    eta = 0.005;
