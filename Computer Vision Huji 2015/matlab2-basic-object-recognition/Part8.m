function [TrnErr,VldErr,minT,minTrnErr,minVldErr,falsePositive,falseNegative] = Part8()
...
onesVectorTrain = ones(600,1);
onesVectorValid = ones(300,1);
for i=1:rows
    trainSetAdabost = trainSet(:,1 + (i-1)*cols:i*cols);
    validSetAdabost = validSet(:,1 + (i-1)*cols:i*cols);
    trainX = [trainSetAdabost onesVectorTrain];
    validX = [validSetAdabost onesVectorValid];
    w = RidgeTrain(trainSetAdabost, trainY, 10^(-2));
    trainClassify(:,i) = trainX * w;
    validClassify(:,i) = validX * w;  
end
